...
from utils import arghandle, coin, graph, misc
...
def plot_diameter(d, w, trials):
    ...
    for x in xs:
    	y = float(coin.run_trials(x, w, trials))/trials
...
def plot_width(d, w, trials):
    ...
    ys = coin.predict_prob(d, xs)
    graph.line_plot(xs, ys)

    for x in xs:
    	y = float(coin.run_trials(d, x, trials))/trials
    	graph.scatter_plot(x, y)
...
def _run_handler(args):
    hits = coin.run_trials(args.diameter, args.gap, args.trials)

    print("%d/%d" % (hits, args.trials))
    prob = misc.get_prob(hits, args.trials)
    print("observed prob: %f" % prob)
    print("expected prob: %f" % coin.predict_prob(args.diameter, args.gap))
